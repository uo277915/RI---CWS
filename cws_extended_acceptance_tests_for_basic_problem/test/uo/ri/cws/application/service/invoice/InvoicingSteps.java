...
public class InvoicingSteps {
	...
	public void iCreateAnInvoiceForTheWorkorders() throws BusinessException {
		if (workordersIds.isEmpty()) {
			workordersIds = ((List<WorkOrderDto>) this.ctx.get(TestContext.Key.WORKORDERS))
			.stream().filter(wo -> wo.status.equals("FINISHED")).map(wo -> wo.id).collect(Collectors.toList());
		}
		invoice = Factory.service.forInvoicingService().createInvoiceFor(workordersIds);
	}
	...
	public void anInvoiceIsCreated() {
		/* invoice is created */
		InvoiceRecord found = new InvoiceUtil().find(invoice.id).get();
	...
	private InvoiceDto createExpectedInvoice(List<WorkOrderDto> workorders) {
		...
		result.status = InvoiceDto.InvoiceStatus.NOT_YET_PAID;
		return result;
	}
